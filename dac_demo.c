    ...
    if (clock100ms_flag == 1)   // ¼ì²éDACµçÑ¹ÊÇ·ñÒª±ä
		{
			...
			if (display_toggle_flag == 1)
			{
				...
				digit[0] = mean_i / 1000. ; 	  // ¼ÆËãÇ§Î»Êı
				digit[1] = mean_i / 100 % 10;   // ¼ÆËã°ÙÎ»Êı
				digit[2] = mean_i / 10 % 10;    // ¼ÆËãÊ®Î»Êı
				digit[3] = mean_i % 10;         // ¼ÆËã¸öÎ»Êı
			...
			*/
			// ÏÔÊ¾DACĞ¾Æ¬Éè¶¨µÄµçÑ
			if (display_toggle_flag == 2){
			digit[4] = mean_i / 1000 ; 	  // ¼ÆËãÇ§Î»Êı
			digit[5] = mean_i / 100 % 10;   // ¼ÆËã°ÙÎ»Êı
			digit[6] = mean_i / 10 % 10;    // ¼ÆËãÊ®Î»Êı
			digit[7] = mean_i % 10;         // ¼ÆËã¸öÎ»Êı
				
			digit[0] = mean_i_c / 1000. ; 	  // ¼ÆËãÇ§Î»Êı
			digit[1] = mean_i_c / 100 % 10;   // ¼ÆËã°ÙÎ»Êı
			digit[2] = mean_i_c / 10 % 10;    // ¼ÆËãÊ®Î»Êı
			digit[3] = mean_i_c % 10;         // ¼ÆËã¸öÎ»Êı
			}
...
void StateMachine(void) {
	...
	*/	
		...
		case 3: // ×´Ì¬Èı 
			...
			if (last_target_i != target_i) { //TODO: Ìí¼ÓÒ»¸öÃÅÏŞ
				if (mean_i > target_i+3 && mean_u < 5200){
					...
					//if (DAC6571_code <= 80 && DAC6571_code >= 72 ) DAC6571_code = 75; // Ç¿ĞĞĞŞÕı
					//if (DAC6571_code <= 125 && DAC6571_code >= 118 ) DAC6571_code = 120; // Ç¿ĞĞĞŞÕı
					clock250ms = 0;
					clock250ms_flag = 0;
					status = 2;
...
//*****************************************************************************
//
// º¯ÊıÔ­ĞÍ£ºvoid ShiftAvarage(void)
// º¯Êı¹¦ÄÜ£º»¬¶¯Æ½¾ù£¬ÇóÈ¡ADCµÄÁ¿
// º¯Êı²ÎÊı£ºÎŞ
// º¯Êı·µ»ØÖµ£ºÎŞ
//
//*****************************************************************************
